...
void sintaxis::printError(errors t_err, Lexem lex) {
    ...
    case MUST_BE_COMP: {
        std::cerr << "<E> Syntax: Debe estar activado el inicio de sesión de comparación "
            << lex.GetLine() << " line" << std::endl;
        break;
    }
    case DUPL_ID_ERR: {
    ...
    case MUST_BE_DOTS_ARR: {
        std::cerr << "<E> Syntax: Definición de rango de matriz no válida, debe ser '..' instead '" << lex.GetName()
            << "' on " << lex.GetLine() << " line" << std::endl;
        break;
    }
...
void sintaxis::updateVarTypes(const std::list<std::string>& t_var_list, const std::string& t_type_name) {
    try {
        ...
        std::cerr << "<E> Syntax: encontrar la excepción en " << __func__ << ": "
            << exp.what() << std::endl;
    }
}
